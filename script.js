...
const originalAnalizarGastos4 = analizarGastos;
analizarGastos = function() {
  originalAnalizarGastos4();
  actualizarResumenFlotante(listaGastos);
}

const originalAnalizarConDatos4 = analizarGastosConDatos;
analizarGastosConDatos = function(gastos) {
  originalAnalizarConDatos4(gastos);
  actualizarResumenFlotante(gastos);
}
...
function guardarEnHistorial(total, restante, estado) {
  const fecha = new Date().toLocaleString("es-MX");
  historialAnalisis.unshift({ fecha, total, restante, estado });
  if (historialAnalisis.length > 5) historialAnalisis.pop();
}

function mostrarHistorial() {
  const lista = document.getElementById("listaHistorial");
  lista.innerHTML = "";
  historialAnalisis.forEach(h => {
    const color = h.estado === "bueno" ? "green" : h.estado === "medio" ? "orange" : "red";
    lista.innerHTML += `<li><strong>${h.fecha}</strong>: Gastaste $${h.total.toFixed(2)}, Restante $${h.restante.toFixed(2)} <span style="color:${color}; font-weight:bold;">[${h.estado.toUpperCase()}]</span></li>`;
  });
}
...
function toggleHistorial() {
  ...
  if (lista.style.display === "block") mostrarHistorial();
}

function resumenYHistorial(gastos) {
  const sueldo = parseFloat(document.getElementById("sueldo").value);
  const totalGastado = gastos.reduce((sum, g) => sum + g.monto, 0);
  const restante = sueldo - totalGastado;
  const resumen = document.getElementById("resumenMes");
  ...
  resumen.textContent = mensaje;
  guardarEnHistorial(totalGastado, restante, estado);
}

// Sustituir llamadas a resumen anterior
const originalAnalizarGastos5 = analizarGastos;
analizarGastos = function() {
  originalAnalizarGastos5();
  resumenYHistorial(listaGastos);
}

const originalAnalizarConDatos5 = analizarGastosConDatos;
analizarGastosConDatos = function(gastos) {
  originalAnalizarConDatos5(gastos);
  resumenYHistorial(gastos);
}
...
// Integrar el toast al agregar gasto
const originalAgregarGasto = agregarGasto;
agregarGasto = function() {
  originalAgregarGasto();
  mostrarToast("✅ Gasto agregado correctamente");
}

// Integrar toast al reiniciar todo (si existe la función)
if (typeof reiniciarTodo === "function") {
  const originalReiniciar = reiniciarTodo;
  reiniciarTodo = function() {
    originalReiniciar();
    mostrarToast("🗑️ Datos reiniciados");
  }
}
...
function exportarAExcel() {
  const ws_data = [["Tipo de gasto", "Fecha", "Monto"]];
  listaGastos.forEach(g => {
    ws_data.push([g.categoria, g.fecha, g.monto]);
  });
...
function compararMeses() {
  ...
  if (!mes1 || !mes2) {
    resultado.textContent = "Selecciona ambos meses para comparar.";
    resultado.style.color = "gray";
    return;
  }

  const total1 = calcularTotalPorMes(mes1);
  const total2 = calcularTotalPorMes(mes2);

  let mensaje = `Total en ${mes1}: $${total1.toFixed(2)} | Total en ${mes2}: $${total2.toFixed(2)}. `;
  if (total1 > total2) {
    mensaje += `⬇️ Gastaste menos en ${mes2}.`;
    resultado.style.color = "green";
  } else if (total1 < total2) {
    mensaje += `⬆️ Gastaste más en ${mes2}.`;
    resultado.style.color = "red";
  } else {
    mensaje += "El gasto fue igual en ambos meses.";
    resultado.style.color = "orange";
  }
...
function calcularTotalPorMes(mes) {
  let total = 0;
  listaGastos.forEach(g => {
    if (g.fecha && g.fecha.startsWith(mes)) {
      total += g.monto;
    }
  });
  return total;
...
const palabrasClave = {
  ...
  "ropa": "Ropa básica",
  "alimento": "Alimentos",
  "restaurante": "Restaurantes económicos",
  "taller": "Mantenimiento auto",
  "corte": "Servicios de belleza",
  "cine": "Cine",
  "apuesta": "Apuestas",
  "cerveza": "Alcohol",
  "cervezas": "Alcohol",
...
document.getElementById("monto").addEventListener("blur", autoCategorizar);

function autoCategorizar() {
  const descripcion = document.getElementById("categoria").value.toLowerCase();
  for (const palabra in palabrasClave) {
    if (descripcion.includes(palabra)) {
      document.getElementById("categoria").value = palabrasClave[palabra];
      break;
    }
  }
}
...
function exportarAExcel() {
  const ws_data = [["Tipo de gasto", "Fecha", "Monto"]];
  const tabla = document.querySelectorAll("#tablaGastos tr");
  tabla.forEach(row => {
    const celdas = row.querySelectorAll("td");
    if (celdas.length === 3) {
      ws_data.push([celdas[0].innerText, celdas[1].innerText, celdas[2].innerText.replace('$', '')]);
    }
  });
...
function actualizarResumenFinanciero(gastos) {
  const sueldo = parseFloat(document.getElementById("sueldo").value);
  if (isNaN(sueldo) || sueldo <= 0) return;

  const totalGastado = gastos.reduce((sum, g) => sum + g.monto, 0);
  const porcentajeUsado = ((totalGastado / sueldo) * 100).toFixed(0);
  const innecesarios = gastos.filter(g => g.categoria.toLowerCase().includes("innecesario") || g.color === "red").length;

  document.getElementById("totalGastadoResumen").textContent = `$${totalGastado.toFixed(2)}`;
  document.getElementById("porcentajeUsadoResumen").textContent = `${porcentajeUsado}%`;
  document.getElementById("gastosInnecesariosResumen").textContent = innecesarios;
}

const originalAnalizarGastos8 = analizarGastos;
analizarGastos = function() {
  originalAnalizarGastos8();
  actualizarResumenFinanciero(listaGastos);
}

const originalAnalizarConDatos8 = analizarGastosConDatos;
analizarGastosConDatos = function(gastos) {
  originalAnalizarConDatos8(gastos);
  actualizarResumenFinanciero(gastos);
}
...
function alternarModoCompacto() {
  const secciones = [
    "grafica-container",
    "graficaPastel-container",
    "metaAhorro-container",
  ...
  secciones.forEach(id => {
    ...
    if (el) {
      el.style.display = modoCompactoActivo ? "block" : "none";
    }
  });

  const boton = document.getElementById("botonModoCompacto");
  if (boton) {
    boton.textContent = modoCompactoActivo ? "🧩 Activar modo compacto" : "🧩 Volver al modo completo";
  }

  modoCompactoActivo = !modoCompactoActivo;
}
